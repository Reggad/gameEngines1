...
void PhysicsFactory::CreateRagDoll(glm::vec3 torsoCenter)
{
	glm::vec3 ra(torsoCenter.x,torsoCenter.y+2, torsoCenter.z+5);
	glm::vec3 la(torsoCenter.x,torsoCenter.y+2, torsoCenter.z-5);
	glm::vec3 rl(torsoCenter.x,torsoCenter.y-6, torsoCenter.z+1);
	glm::vec3 ll(torsoCenter.x,torsoCenter.y-6, torsoCenter.z-1);
	glm::vec3 h(torsoCenter.x,torsoCenter.y+4, torsoCenter.z);


	shared_ptr<PhysicsController> torso = CreateBox(1, 5, 2, torsoCenter, glm::quat());
	shared_ptr<PhysicsController> rightArm = CreateBox(1, 1, 4, ra, glm::quat()) ;
	shared_ptr<PhysicsController> leftArm = CreateBox(1, 1, 4, la, glm::quat()) ;
	shared_ptr<PhysicsController> rightLeg =  CreateBox(1, 5, 1, rl, glm::quat());
	shared_ptr<PhysicsController> leftLeg =  CreateBox(1, 5, 1, ll, glm::quat());
	shared_ptr<PhysicsController> head = CreateBox(1, 1, 1, h, glm::quat()) ;


	btPoint2PointConstraint * ptpConstraint = new btPoint2PointConstraint(*torso->rigidBody, *rightArm->rigidBody, btVector3(0,0,2.5f),btVector3(0,0,-2.5f));
	dynamicsWorld->addConstraint(ptpConstraint);
	ptpConstraint = new btPoint2PointConstraint(*torso->rigidBody, *leftArm->rigidBody, btVector3(0,0,-2.5f),btVector3(0,0,2.5f));
	dynamicsWorld->addConstraint(ptpConstraint);
	ptpConstraint = new btPoint2PointConstraint(*torso->rigidBody, *rightLeg->rigidBody, btVector3(0,-3.5,0.5f),btVector3(0,3.5,0.5f));
	dynamicsWorld->addConstraint(ptpConstraint);
	ptpConstraint = new btPoint2PointConstraint(*torso->rigidBody, *leftLeg->rigidBody, btVector3(0,-3.5,-0.5f),btVector3(0,3.5,-0.5f));
	dynamicsWorld->addConstraint(ptpConstraint);
	ptpConstraint = new btPoint2PointConstraint(*torso->rigidBody, *head->rigidBody, btVector3(0,3,0.0f),btVector3(0,-1.5,0.0f));
	dynamicsWorld->addConstraint(ptpConstraint);
...
shared_ptr<PhysicsController> PhysicsFactory::CreateTBox(float width, float height, float depth, float massOfBox, glm::vec3 pos, glm::quat quat)
{
        // Create the shape
        btCollisionShape * boxShape = new btBoxShape(btVector3(width, height, depth) * 0.5);
        btScalar mass = massOfBox;
        btVector3 boxInertia(0,0,0);
        boxShape->calculateLocalInertia(mass,boxInertia);
